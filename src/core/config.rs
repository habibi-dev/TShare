...
type RedisConn = Arc<ConnectionManager>;
...
impl Config {
    ...
    pub async fn setup_database() -> Result<sea_orm::DatabaseConnection> {
        ...
        // Limit SQLite page cache to 2MB
        db.execute_unprepared("PRAGMA cache_size = -2000;")
            .await
    ...
    fn build_connect_options(database_url: &str) -> ConnectOptions {
        ...
        options
            .max_connections(1)
            .min_connections(1)
            // .max_connections(20) FOR POSTGRESQL OR MYSQL
            // .min_connections(5) FOR POSTGRESQL OR MYSQL
